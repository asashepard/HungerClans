...
public class WarManager {
    ...
    public void endWar(String war) {
        ...
        for(String uuid : plugin.getDataManager().getConfig().getConfigurationSection("clans." + winner).getStringList("members")) {
            ...
            if(oPlayer.isOnline()) {
                ...
                player.sendMessage(TextUtil.convertColor("&4&lCLANS &8» &aCongratulations, your clan won a war against " + color + loser + "&a! &3" +
        ...
        for(String uuid : plugin.getDataManager().getConfig().getConfigurationSection("clans." + loser).getStringList("members")) {
            ...
            if(oPlayer.isOnline()) {
                Player player = (Player) oPlayer;
                player.sendMessage(TextUtil.convertColor("&4&lCLANS &8» &4Your clan lost a war against " + wColor + winner + "&4. " +
    ...
    public void endWar(String war, String winner, boolean surrender) { //if not surrender, other team disbanded
        ...
        for(String uuid : plugin.getDataManager().getConfig().getConfigurationSection("clans." + winner).getStringList("members")) {
            ...
            if(oPlayer.isOnline()) {
                Player player = (Player) oPlayer;
                player.sendMessage(TextUtil.convertColor("&4&lCLANS &8» &aCongratulations, your clan won a war! &3" +
        ...
        if(surrender) {
            ...
            for(String uuid : plugin.getDataManager().getConfig().getConfigurationSection("clans." + loser).getStringList("members")) {
                ...
                if(oPlayer.isOnline()) {
                    Player player = (Player) oPlayer;
                    player.sendMessage(TextUtil.convertColor("&4&lCLANS &8» &4Your clan surrendered and lost a war against " + wColor + winner + "&4."));
                }
                else {
